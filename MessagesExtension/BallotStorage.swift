...
class BallotStorage {
    private(set) var savedBallots: [Ballot]

    init() {
        savedBallots = BallotStorage.defaultBallots
    }

    func save(_ ballot: Ballot) {
        savedBallots.insert(ballot, at: 0)
    }

    static var defaultBallots: [Ballot] {
        return [
            Ballot(
                questionText: nil,
                candidates: [
